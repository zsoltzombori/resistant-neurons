...
AUGMENTATION = False
SESSION_NAME = "{}".format(time.strftime('%Y%m%d-%H%M%S'))
BN_WEIGHT = 0
COV_WEIGHT = 0
...
(X_train, y_train), (X_devel, y_devel), (X_test, y_test) = data.load_data(DATASET, SEED, USEFULNESS_EVAL_SET_SIZE)
...
for iteration in range(ITERS+1):
    ...
    if iteration % EVALUATION_CHECKPOINT == 0:
        ...
        saver.save(session, f'saved_models/model_{SESSION_NAME}', global_step=iteration)
        # print(_total_loss)
