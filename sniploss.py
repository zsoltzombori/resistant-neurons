...
AUGMENTATION = False
SESSION_NAME = f'{time.strftime("%Y%m%d-%H%M%S")}_noWD_{ITERS}'
BN_WEIGHT = 0
COV_WEIGHT = 0
LOG_DIR = "logs/%s" % SESSION_NAME
EVALUATE_USEFULNESS = True
USEFULNESS_EVAL_SET_SIZE = 1000
SAVE_ACTIVATIONS = 0  # number of batches to save
BASE_SAVEPATH = '/home/levaid/bigstorage/neuron_logs'

device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
torch.multiprocessing.set_sharing_strategy('file_system')  # there is some weird bug with the loader
starttime = time.time()
...
def get_activation(name):
    def hook(model, input, output):
        if len(hidden_activations[name]) < SAVE_ACTIVATIONS and not model.training:
...
for epoch in range(ITERS):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(vanish_dataloader, 0):
        ...
        if True:
            ...
            sniploss = 0
            for name, layer in net.named_parameters():
                if layer.requires_grad and 'conv' in name:
                    # print(f'here we go, {name}')
                    # print(layer.grad.shape, layer.shape)
                    ex_tensor = layer.grad.data * layer
                    # print(ex_tensor.shape)
                    norm = torch.norm(ex_tensor, p=2)
                    # print(norm.shape, norm)
                    sniploss += SNIP_NORM * norm
    ...
    Weight sum: {sum([p.abs().sum() for p in net.parameters()]):.3f}\tLR: {float(param_group["lr"]):.3f}')
