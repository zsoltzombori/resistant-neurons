...
if CLASSIFIER_TYPE == "dense":
    output, activations, zs = networks.DenseNet(inputs, DEPTH, WIDTH, BN_DO, OUTPUT_COUNT, dropout=0.5)
elif CLASSIFIER_TYPE == "conv":
    output, activations, zs = networks.LeNet(inputs, BN_DO, OUTPUT_COUNT, dropout=0.8)
...
if BN_WEIGHT > 0: # push each neuron to have zero mean and unit variance output
...
kernels = [v for v in tf.trainable_variables() if (v.name.find("dense_") == 0) and v.name.find("kernel") > 0]
...
def evaluate(Xs, ys, BATCH_SIZE):
    ...
    for X_batch, y_batch in eval_gen:
        value_list = session.run([total_loss, output, activations] + list(cov_ops), feed_dict={inputs:X_batch, labels:y_batch})
